...
void database()
{
	...
	if (!mysql_real_connect(mySQL, "192.168.43.22", "root", "", "monitoring", 0, NULL, 0)) {
		printf("Une erreur s'est produite lors de la connexion à la BDD!");
	}
	else {
		mysql_query(mySQL, "UPDATE route SET nb_v = nb_v+1");
	}
}
...
void VehicleCouting::process()
{
  ...
  if(FAV1::use_roi == true && FAV1::roi_defined == false && firstTime == true)
  {
    do
    {
      cv::putText(img_input, "Tracer le counting line", cv::Point(10,15), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0,0,255));
  ...
  if (FAV1::use_roi == true && FAV1::roi_defined == true)
    cv::line(img_input, cv::Point(FAV1::roi_x0,FAV1::roi_y0), cv::Point(FAV1::roi_x1,FAV1::roi_y1), cv::Scalar(100,80,40));
    ...
    if(points.count(id) > 0)
    {
      ...
      if(track->inactive == 0 && centroids.size() < 30)
      {
        ...
        for(std::vector<CvPoint2D64f>::iterator it3 = centroids2.begin() ; it3 != centroids2.end(); it3++)
        {
          ...
          cv::circle(img_input, cv::Point((*it3).x,(*it3).y), 3, cv::Scalar(50,255,50), -1);
        }
  ...
  std::string countABstr;
	  countABstr = "Compteur : " + std::to_string(countAB);
  std::string countBAstr;
	 countBAstr = "Compteur : " + std::to_string(countBA);
  ...
  if(showAB == 1)
    cv::putText(img_input, countABstr, cv::Point(10, img_h - 8), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(47, 0, 0));
  
  if(showAB == 2)
    cv::putText(img_input, countBAstr, cv::Point(10, img_h - 8), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));

  if(showOutput)
    cv::imshow("Compteur", img_input);
